---
title: Testing onSubmit forms
description: Testing a form created with Createform in onSubmit mode is easy and straightforward. Here are some tips for testing your forms.
tags: react, form, useform, createform, use-form, hook, testing, testing-lirary
---

# Testing onSubmit forms
...
Createform is a powerful library for creating forms in React. One of the modes that Createform supports is onSubmit, which means that the form will only show error messages when the form is submitted. This mode is useful for validating user input before submitting the form.

Testing a form created with Createform in onSubmit mode is easy and straightforward. Here are some tips for testing your forms:
...
2 - Define the form using `createForm` with the `onSubmit` mode:
...
<Callout type="info">
  In `onSubmit` mode, validation is triggered after the submit event, which means that we need to simulate this event in order to test if the expected behavior is true.
</Callout>
