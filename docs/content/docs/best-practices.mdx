...
```tsx
// ✅ Good: createForm() for complex, reusable forms
...
CreateForm is designed for minimal impact:

- **3.3kB gzipped** (10.6kB minified) - Much smaller than alternatives
...
// ❌ Don't import the entire library if using bundlers without tree-shaking
import * as CreateForm from "@createform/react";
```
...
```tsx
// 🔥 Best performance: onSubmit (default)
const useForm = createForm({
  mode: 'onSubmit', // Only validates on submit
  // ... other options
});

// ⚖️ Balanced: onChange for better UX
const useForm = createForm({
  mode: 'onChange', // Validates as user types
  // ... other options
});

// 🐌 Use sparingly: debounce
const useForm = createForm({
  mode: 'debounce', // Validates after user stops typing (500ms delay)
  // ... other options
});
```
...
// ✅ Also good: Using createForm (recommended)
...
```tsx
// ✅ Good: Use inline validation for dynamic rules
<input 
  {...register({
    name: "username",
...
```tsx
// ✅ Good: Handle submission errors gracefully
...
```tsx
// ✅ Good: Wrapper for third-party components
...
```tsx
// ✅ Good: Separate file for form logic
...
### ❌ Don't: Create forms inside render
```tsx
// ❌ Bad
