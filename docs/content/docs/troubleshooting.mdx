...
1. **Wrong mode configuration**
   ```tsx
   // ❌ Problem: onSubmit mode won't update during typing
   const useForm = createForm({
     mode: 'onSubmit', // Only updates on submit
   });

   // ✅ Solution: Use onChange mode for real-time updates
   const useForm = createForm({
...
2. **Missing register function**
   ...
   // ✅ Solution: Use register function
   <input {...register('email')} />
   ```

3. **Custom component not forwarding ref**
   ```tsx
   // ❌ Problem: Custom component doesn't forward ref
...
2. **Ensure validation mode is correct**
   ```tsx
   // ❌ Problem: Validation only runs on submit
   const useForm = createForm({
     mode: 'onSubmit',
     validationSchema: schema,
   });

   // ✅ Solution: Use onChange for immediate validation
   const useForm = createForm({
     mode: 'onChange',
     validationSchema: schema,
   });
...
1. **Use appropriate mode**
   ```tsx
   // 🐌 Slow: onChange mode with complex validation
   const useForm = createForm({
     mode: 'onChange',
     validationSchema: expensiveValidation,
   });

   // ⚡ Faster: debounced mode
   const useForm = createForm({
     mode: 'debounced',
     debounceMs: 300,
     validationSchema: expensiveValidation,
...
2. **Optimize validation**
   ...
   // ✅ Better: Use inline validation with debounce
   <input 
     {...register({
       name: 'username',
...
// ✅ Or use effect to load from localStorage
useEffect(() => {
...
- 📚 [Documentation](/docs)
- 💬 [GitHub Discussions](https://github.com/jucian0/createform/discussions)
- 🐛 [Issue Tracker](https://github.com/jucian0/createform/issues)
- 📦 [Examples Repository](https://github.com/jucian0/createform/tree/main/examples)

---

Still having issues? Don't hesitate to [open an issue](https://github.com/jucian0/createform/issues/new) or start a [discussion](https://github.com/jucian0/createform/discussions). We're here to help! 🚀