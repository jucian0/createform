...
This function sets the specific field as touched.
...
This function resets the touched property of a specific field. Should receive a field name, if the field was not declared in `initialTouched` this field will receive `false` as a touched property.
...
This function sets an error in a specific field. The first parameter is a field name, if using a object with many layers should use a dot notation to specific correct path of field, `ex: 'person.name'` and the second parameter is a message error.
...
This function resets the error of a specific field. Should receive a field name.
